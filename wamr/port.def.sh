...
#shellcheck disable=2034
{
	...
	# @FIXME(michal.lach): setting `supports` to an empty string disables
	# Phoenix version requirement in dependency resolution.
	# This is because we don't have a way to express/tag what version
	# master currently tracks to release.
	# This is unsupported and should not be normally done.
	# This port will not build on phoenix<=3.3.
	supports=""
...
p_prepare() {
	...
	arm*)
		CMAKE_FLAGS+=("-DWAMR_BUILD_TARGET=THUMB" "-DWAMR_BUILD_INVOKE_NATIVE_GENERAL=1")
		if [[ "${TARGET_FAMILY}" =~ "armv7m" || "${TARGET_FAMILY}" =~ "armv8m" ]]; then
			CMAKE_FLAGS+=("-DTARGET_HAS_TLS=1")
		fi
		;;
	aarch64*)
		CMAKE_FLAGS+=("-DWAMR_BUILD_TARGET=AARCH64")
		;;
	riscv*)
		CMAKE_FLAGS+=("-DWAMR_BUILD_TARGET=RISCV64")
		;;
	*)
	...
	if b_use "low_memory"; then
		CMAKE_FLAGS+=("-DWAMR_BUILD_LIBC_WASI=0" "-DWAMR_BUILD_FAST_INTERP=0" "-DWAMR_BUILD_SIMD=0" "-DWAMR_BUILD_LIB_WASI_THREADS=0")
		WAMR_THREAD_STACK_SIZE_MAX="131072" # As per recommendations from WAMR build documentation
	else
		CMAKE_FLAGS+=("-DWAMR_BUILD_LIB_WASI_THREADS=1" "-DWAMR_BUILD_BULK_MEMORY=1" "-DWAMR_BUILD_LIB_SIMDE=1" "-DWAMR_BUILD_SIMD=1")
	fi

	CMAKE_FLAGS+=("-DWAMR_APP_THREAD_STACK_SIZE_MAX=${WAMR_THREAD_STACK_SIZE_MAX}")

	mkdir -p "${PREFIX_PORT_WORKDIR}/product-mini/platforms/phoenix/build"
	(cd "${PREFIX_PORT_WORKDIR}/product-mini/platforms/phoenix/build" && cmake "${CMAKE_FLAGS[@]}" ..)
}
