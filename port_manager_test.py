...
from port_manager_v2 import DependencyManager, LogLevel, logger
...
def fix():
    ...
    logger.set_level(LogLevel.VERBOSE if os.getenv(
...
def test_args_bad(fix):
    ...
    pytest.raises(Exception, Utils.discover_port, pm,
...
def test_database_simple(fix):
    with tempfile.TemporaryDirectory() as db_path:
        ...
        Utils.discover_port(pm, port=foo, depends=["bar>=1.1.1"],
                            db=db_path)
        Utils.discover_port(pm, port=bar, db=db_path)
        ...
        pm = DependencyManager()
        Utils.resolve(pm, ports=[foo], db=db_path)
        assert [PREFIX_BUILD] == Utils.dep_install_path(
            pm, port=foo, dep_name="bar", db=db_path)
...
def test_install_simple(fix):
    ...
    assert [os.path.join(PREFIX_PORTS, bar)
            ] == Utils.list_deps_to_install(pm, foo)
