...
mkdir -p "${PREFIX_LSB_VSX_BUILD}/host_config"
mkdir -p "${PREFIX_LSB_VSX_BUILD}/ps_config"
...
wget "$PKG_URL1" -P "${PREFIX_LSB_VSX}/packages" || wget "$PKG_MIRROR_URL1" -P "${PREFIX_LSB_VSX}/packages"

[ -f "${PREFIX_LSB_VSX}/packages/lts_vsx-pcts2.0beta.tgz" ] || \
wget "$PKG_URL2" -P "${PREFIX_LSB_VSX}/packages" || wget "$PKG_MIRROR_URL2" -P "${PREFIX_LSB_VSX}/packages"
...
if [ ! -f "${PREFIX_LSB_VSX_MARKERS}/host_VSX/host_VSX.built" ]; then
	...
	cp -p "${HOME}/tetbuild.cfg" "${PREFIX_LSB_VSX_BUILD}/host_config/tetbuild.cfg"
...
sed -e "s|^CC=.*$|CC=\"${CC}\"|" \
    ...
    "${PREFIX_LSB_VSX}/config/ps_vsxparams.skel" > "${PREFIX_LSB_VSX_BUILD}/ps_config/ps_vsxparams"

#
# # Build VSX test framework
#

export PREFIX_LSB_VSX_BUILD
...
then
	...
	sed -e "s|^PATH=.*$|PATH=${PATH}|" \
	    ...
	    "${PREFIX_LSB_VSX_BUILD}/host_config/tetbuild.cfg" > "${HOME}/tetbuild.cfg"
	cd "$HOME" && "${PREFIX_LSB_VSX_BUILD}/host_bin/tcc" -p -b -s "${PREFIX_LSB_VSX}/config/scen.bld"
...
#
	...
	cp -p "${PREFIX_LSB_VSX}/config/ps_tetexec.cfg" "${PREFIX_ROOTFS}/root/lsb_vsx/test_sets/tetexec.cfg"
