...
PREFIX_UPYTH_SRC=${PREFIX_PORT_BUILD}/${UPYTH}
PREFIX_UPYTH_CONFIG="${PREFIX_PORT}/${UPYTH}-config/"
PREFIX_UPYTH_MARKERS="$PREFIX_PORT_BUILD/markers/"
...
# Download and unpack
#
mkdir -p "$PREFIX_PORT_BUILD" "$PREFIX_UPYTH_MARKERS"
if [ ! -f "$PREFIX_PORT/${UPYTH}.tar.xz" ]; then
	if ! wget "$PKG_URL" -P "${PREFIX_PORT}" --no-check-certificate; then
		wget "$PKG_MIRROR_URL" -P "${PREFIX_PORT}" --no-check-certificate
	fi
fi
[ -d "${PREFIX_UPYTH_SRC}" ] || tar xf "$PREFIX_PORT/${UPYTH}.tar.xz" -C "$PREFIX_PORT_BUILD"

#
# Build mpy-cross for host if not already built
#
if [ ! -f "${PREFIX_UPYTH_SRC}/mpy-cross/build/mpy-cross" ]; then
	(cd "${PREFIX_UPYTH_SRC}/ports/unix" && CFLAGS="" LDFLAGS="" make -C ../../mpy-cross all DEBUG=1 V=1)
fi
...
cp -a "${PREFIX_UPYTH_SRC}/ports/unix/build-standard/micropython" "$PREFIX_PROG_STRIPPED"
...
if [ "$LONG_TEST" = "y" ]; then
	...
	do
		...
		for test in "$PREFIX_UPYTH_TESTS_DES"/"$des_dir"/*.py; do
			...
			cd "$PREFIX_UPYTH_TESTS_DES" && $CPYTHON3_CMD "$des_dir/$(basename "$test")" > "$test.exp" 2>&1  || true
			sed -i 's/\r\n/\n/g' "$test.exp"
		done
	done
fi
