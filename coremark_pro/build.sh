...
# FIXME: bignum symbols leak from libphoenix (issue #1275)
find -name "*.c" -exec sed -i 's/bignum_/_bignum_/g' {} \;
find -name "*.h" -exec sed -i 's/bignum_/_bignum_/g' {} \;

# Build coremark
make TARGET="${HOST}" build
rmdir "${PREFIX_PORT_SRC}/builds/${HOST}/phoenix-gcc/bin/data"
mkdir -p "${PREFIX_PROG}/coremark-pro"
cp -a "${PREFIX_PORT_SRC}/builds/${HOST}/phoenix-gcc/bin/"* \
	"${PREFIX_PROG}/coremark-pro/"
...
b_install "${PREFIX_PORT_SRC}/builds/${HOST}/phoenix-gcc/data/libbmp/"* \
	/data/libbmp/
