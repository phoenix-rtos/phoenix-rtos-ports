...
from typing import (
    Any, Protocol, Mapping, Iterator, Sequence, TypeVar, NamedTuple,
    Generic, Union, Callable, Tuple, Set, Sized
)
...
PreferenceInformation = resolvelib.structs.RequirementInformation[Requirement, Candidate]
...
class VersionGrammar:
    ...
    version = (
        ...
        .set_name("version")
        .set_parse_action(pp.token_map(Version))
    )
...
class PhxProvider(resolvelib.AbstractProvider):
    ...
    def find_matches(
        ...
        logger.debug("FIND MATCHES", identifier, requirements)
        ...
        for candidate in self.all_candidates[identifier].values():
            ...
            if candidate in incompatibilities.values():
                continue
            good = True
            logger.debug(candidate, "conflicts:", candidate.iter_conflicts())
            for conflict in candidate.iter_conflicts():
                if conflict.cname in requirements:
                    logger.error(candidate, "conflicts with", conflict.cname,
    ...
    def get_dependencies(self, candidate: Candidate) -> Iterable[Requirement]:
        logger.debug("MASKED:", self.masked_requirements)
        return (r for r in candidate.iter_dependencies()
                if r is not None
                if r not in self.masked_requirements)


def constraint_satisfied(candidate_version: Version, constraint: Tuple[str, Version]):
...
class BaseRequirement(Requirement):
    def __init__(self, name: str, constraints: Iterable[Tuple[str, Version]]):
        self._name = name
        self.constraints = constraints

    def __repr__(self):
        return self._name + ','.join([rel + str(ver) for (rel, ver) in self.constraints])
...
class ConflictRequirement(BaseRequirement):
    def __init__(self, name: str, cname: str, constraints: Iterable[Tuple[str, Version]]):
...
def parse_namever(namever: str) -> Tuple[str, Version]:
    elems = namever.split('-')
    if len(elems) != 2:
        raise ValueError(
            f"bad name-ver - expected NAME-VERSION, got '{namever}'")
    return (elems[0], Version(elems[1]))
...
class Logger:
    ...
    def _print(self, fmt: str, level: LogLevel, color: Fore, sep: str = " ", **kwargs):
        if level.value >= self.print_level.value:
            print(color + f"{level.name}: " +
                  Style.RESET_ALL + fmt, file=sys.stderr, **kwargs)

    def set_level(self, n: LogLevel) -> None:
        self.print_level = n

    def debug(self, *fmt: object, sep: str = " ", **kwargs) -> None:
        self._print(sep.join(map(str, fmt)), level=LogLevel.VERBOSE,
                    color=Fore.GREEN, **kwargs)

    def info(self, *fmt: object, sep: str = " ", **kwargs) -> None:
        self._print(sep.join(map(str, fmt)), level=LogLevel.INFO,
                    color=Fore.CYAN, **kwargs)

    def warning(self, *fmt: object, sep: str = " ", **kwargs) -> None:
        self._print(sep.join(map(str, fmt)), level=LogLevel.WARNING,
                    color=Fore.YELLOW, **kwargs)

    def error(self, *fmt: object, sep: str = " ", **kwargs) -> None:
        self._print(sep.join(map(str, fmt)), level=LogLevel.ERROR,
                    color=Fore.RED, **kwargs)
...
class MyReporter(resolvelib.BaseReporter):
    ...
    def rejecting_candidate(self, criterion: resolvelib.structs.Criterion[RT, CT], candidate: CT) -> None:
        for req_info in criterion.information:
            ...
            if isinstance(req, OptionalRequirement):
                ...
                logger.debug(
                    f"{parent} requirement for {req} unsatisfiable")
...
class DependencyManager():
    ...
    def read_candidates(self) -> None:
        if self.db_path:
            path = self.get_db_file_path(DependencyManager.CANDIDATES_FILE)
            if os.path.exists(path):
                with open(path, "r") as f:
                    self.candidates = jsonpickle.decode(f.read())
    ...
    def write_candidates(self) -> None:
        if self.db_path:
            ...
            with open(path, "w+") as f:
                json_text = jsonpickle.encode(self.candidates, indent=2)
                f.write(json_text)

    def read_mapping(self) -> None:
        if self.db_path:
            path = self.get_db_file_path(DependencyManager.TREE_FILE)
            with open(path, "r") as f:
                self.mapping = jsonpickle.decode(f.read())
    ...
    def write_mapping(self) -> None:
        if self.db_path:
            ...
            path = self.get_db_file_path(DependencyManager.TREE_FILE)
            with open(path, "w+") as f:
                json_text = jsonpickle.encode(self.mapping, indent=2)
                f.write(json_text)
    ...
    def lookup_candidate(self, name: str, version: Version, candidate_type: type | None = None) -> Candidate:
    ...
    def discover(self, args: Namespace) -> None:
        ...
        def safe_join(s): return ' '.join(s) if s else ''
        ...
        conflicts = parse_requirements(safe_join(args.conflicts), lambda rname, constraints:
                                       ConflictRequirement(name, rname, constraints))

        self.add_candidate(InstallableCandidate(name, version, req, conflicts,
                                                args.def_dir))

        self.write_candidates()

    def resolve(self, args: Namespace) -> None:
    ...
    def query(self, args: Namespace) -> None:
        ...
        def print_candidate(candidate: Candidate, indent=0):
            def iprint(*args): print(indent * " " + " ".join(map(str, args)))
            iprint("name:", name)
        ...
        try:
            ...
            logger.error(f"package/dependency unrecognized: {e}" +
                         " - hint: did you run 'resolve' command before querying?")
            sys.exit(1)

    def installed(self, args: Namespace) -> None:
    ...
    def get_argument_parser(self) -> ArgumentParser:
        ...
        resolve.set_defaults(func=self.resolve)
